...
for ep in range(args.epochs):
    ...
    for batch, (images,labels) in enumerate(dataset.take(nstep)):
        ...
        loss_value= hvd.allreduce(acc, average=True)
        acc= hvd.allreduce(acc, average=True)
        
        if (nstep==0 and ep==0 and hdv.rank()==0):
        ...
        if (batch % 100 == 0) and (hvd.rank() == 0):
            ...
            print(f"Epoch - {ep:d}, step: {batch:d}/{nstep:d}  loss:{loss_value:f}")
    ...
    if (hvd.rank() == 0):
        ...
        np.savetxt(f"metrics{hvd.size()}.dat", np.array([metrics['train_acc'], metrics['train_loss'], metrics['valid_acc'], metrics['valid_loss'], metrics['time_per_epochs']]).transpose())
    t1 = time.time()
